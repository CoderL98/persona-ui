...
test.describe("Breadcrumb Visual Regression", () => {
  ...
  for (const themeMode of themes) {
    test(`Breadcrumb renders in ${themeMode}`, async ({ page }) => {
      ...
      const section = page.locator('section[id="breadcrumb"]');
      await expect(section).toBeVisible();
      await expect(section).toHaveScreenshot(`breadcrumb-${themeMode}.png`, {
        maxDiffPixels: 100,
      });
    });
  }
