...
test.describe("Skeleton Visual Regression", () => {
  ...
  for (const themeMode of themes) {
    test(`Skeleton renders in ${themeMode}`, async ({ page }) => {
      ...
      const section = page.locator('section[id="skeleton"]');
      await expect(section).toBeVisible();
      await expect(section).toHaveScreenshot(`skeleton-${themeMode}.png`, {
        maxDiffPixels: 100,
      });
    });
  }
