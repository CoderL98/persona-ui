...
- **BREAKING — 事件回调 prop 重命名为驼峰**（按 `CLAUDE.md` 第 2 条要求）
  ...
  - `TextField`：`oninput` / `onchange` → `onInput` / `onChange`
  - Svelte 5 中 DOM 元素（`<button>`、`<input>` 等）的 `onclick` 属性保持小写不变；仅组件 prop 重命名
  - 迁移：`<Button onclick={fn}>` → `<Button onClick={fn}>`；`<TextField oninput={fn}>` → `<TextField onInput={fn}>`
...
- **feat(components): 新增 Fab、Snackbar 组件及 SectionHeading 文档**（commit `88cbcfb`）
  - `Fab` / `FabProps` / `FabSize` / `FabVariant`
  - `Snackbar` / `SnackbarProps` / `SnackbarTone` / `SnackbarKind` / `SnackbarItem`
  - `triggerHaptic(type)` / `supportsHaptics()` 公开工具，`HapticType` 类型
  - SectionHeading 文档（三语）

### Patch Changes

- fix: 27 个组件用 `crypto.randomUUID()` 替代 `Math.random()` 生成 fallback id，避免 SSR/CSR hydration mismatch 与 ARIA 关联断裂
- refactor: 28 个组件改用 Svelte 5.20+ 内置 `$props.id()` 替代自建 `uniqueId()`，编译时位置 hash 稳定 SSR/CSR
- fix(toast): 修复 ToastViewport `$effect` cleanup 误清除所有 toast timer 的 bug；Toast 单例 timer 改用 `untrack` 写入，闭包不再持有旧值
- fix(toast): `toast.store.ts` ID 生成改用 `crypto.randomUUID()`，`Date.now()` fallback 仍保留为老浏览器降级
- fix(list): 实装 `variant="inset"` 变体（原实现被静默忽略）
- fix(search): 删除 SearchModal input 上重复的 `onkeydown={handleKey}`，避免与 `svelte:window` 双绑导致 ArrowDown/Up 跳 2 步、Enter 触发 2 次 goto
...
- fix(i18n): `store.svelte.ts` `setLocale` 加 `if (browser)` 守卫，避免 SSR 读 `localStorage` 抛错
- fix(sitemap): hreflang 改 BCP 47 标准 `zh-CN` / `zh-TW`（之前使用非规范 `zh-cn` / `zh-tw`）
- fix(404): docs 站新增品牌化错误页（75 行，三语 i18n 齐全）
- fix(calendar): `viewDate` 默认值改 `new Date(2000, 0, 1)`，onMount 后再覆盖为 `new Date()`，避免 SSR 时间不同导致的 hydration mismatch
...
- fix(tailwind): 15 处自检遗漏的 `py-[var()]` / `px-[var()]` / `bg-[var()]` / `rounded-[var()]` 等简单 var() 引用改简写（Textarea / Radio / Badge / Checkbox / Avatar 共 15 行）
...
- chore: `.gitignore` 添加 `*.png` 过滤
- chore: 添加 publish-theme.sh 主题包发布脚本与共享库 `_publish-common.sh`（commit `463f4e4`）
