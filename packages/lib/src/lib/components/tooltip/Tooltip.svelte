<script lang="ts">
  ...
  const placementStyles: Record<string, { tooltip: string; arrow: string }> = {
    top: { tooltip: 'bottom-full left-1/2 -translate-x-1/2 mb-2', arrow: 'top-full left-1/2 -translate-x-1/2 border-t-[var(--pui-surface-raised)]' },
    bottom: { tooltip: 'top-full left-1/2 -translate-x-1/2 mt-2', arrow: 'bottom-full left-1/2 -translate-x-1/2 border-b-[var(--pui-surface-raised)]' },
    left: { tooltip: 'right-full top-1/2 -translate-y-1/2 mr-2', arrow: 'left-full top-1/2 -translate-y-1/2 border-l-[var(--pui-surface-raised)]' },
...
<span role="presentation" class={cn('pui-tooltip-wrapper relative inline-flex', className)} style={style} data-testid={dataTestId}
  ...
  {#if visible}
    <span id={tooltipId} role="tooltip" transition:fade={{ duration: 150 }}
      class={cn('absolute z-(--pui-z-overlay) pointer-events-none whitespace-nowrap rounded-(--pui-radius-control) px-2 py-1 text-xs font-medium shadow-[var(--pui-elevation-2)] bg-[var(--pui-surface-raised)] text-(--pui-text-primary) border border-(--pui-outline-subtle)', ps.tooltip)}>
      {#if isStringContent}
        {content}
      {:else}
