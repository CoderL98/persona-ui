...
<div {...rest} {...attrs} id={id} class={cn('pui-textarea relative flex flex-col gap-1', className)} style={style} data-testid={dataTestId}>
  ...
  <div class={cn('flex items-start gap-2 bg-(--pui-field-bg,var(--pui-surface-base)) rounded-(--pui-field-radius,var(--pui-radius-control)) px-(--pui-field-padding-x,var(--pui-space-3)) py-(--pui-field-padding-y,var(--pui-space-2)) border', error ? 'border-(--pui-color-error)' : 'border-(--pui-field-border,var(--pui-outline))', disabled && 'opacity-(--pui-opacity-disabled) cursor-not-allowed', 'transition-colors', !disabled && 'focus-within:border-(--pui-color-primary)')}>
